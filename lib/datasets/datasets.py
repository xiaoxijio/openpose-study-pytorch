...
class CocoKeypoints(torch.utils.data.Dataset):
    ...
    def add_neck(self, keypoint):
        ...
        right_shoulder = keypoint[6, :]  # 右肩膀
        left_shoulder = keypoint[5, :]  # 左肩膀
        neck = (right_shoulder + left_shoulder) / 2  # 得到脖子
    ...
    def get_ground_truth(self, anns):
        ...
        grid_y = int(self.input_y / self.stride)  # 原始图位置映射到特征图位置
        grid_x = int(self.input_x / self.stride)
        channels_heat = (self.HEATMAP_COUNT + 1)  # 多了个脖子 再加了个背景
        channels_paf = 2 * len(self.LIMB_IDS)  # 初始化躯干连接的向量(x ,y)
        heatmaps = np.zeros((int(grid_y), int(grid_x), channels_heat))  # 初始化高斯热力图
        pafs = np.zeros((int(grid_y), int(grid_x), channels_paf))

        keypoints = []
        for ann in anns:
            single_keypoints = np.array(ann['keypoints']).reshape(17, 3)
            single_keypoints = self.add_neck(single_keypoints)  # 添加脖子
            keypoints.append(single_keypoints)
        keypoints = np.array(keypoints)
        keypoints = self.remove_illegal_joint(keypoints)  # 过滤标记错误的点
        ...
        # pafs
        for i, (k1, k2) in enumerate(self.LIMB_IDS):  # 遍历每个连接的躯干
            # 只计算相同的躯干叠加 不同的躯干不叠加 所以每次遍历其它躯干时会置0
            count = np.zeros((int(grid_y), int(grid_x)), dtype=np.uint32)  # 几个人的躯干重叠在了一起 计算重叠了几层
            for joint in keypoints:  # 遍历每个人
                if joint[k1, 2] > 0.5 and joint[k2, 2] > 0.5:  # 判断这两个点是否都有标注
                    centerA = joint[k1, :2]
                    centerB = joint[k2, :2]
                    vec_map = pafs[:, :, 2 * i:2 * (i + 1)]  # 每一个躯干位置，需要构建x和y两个方向，所以*2 一共会构建38=2*19
