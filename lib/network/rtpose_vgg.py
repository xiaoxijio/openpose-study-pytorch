...
def get_model(trunk='vgg19'):
    ...
    class rtpose_model(nn.Module):
        ...
        def forward(self, x):
            saved_for_loss = []
            out1 = self.model0(x)  # 疯狂卷积relu池化，然后得到46*46的特征图
            out1_1 = self.model1_1(out1)  # PAF输出 38个
            out1_2 = self.model1_2(out1)  # 关节点输出 19个
            out2 = torch.cat([out1_1, out1_2, out1], 1)
            saved_for_loss.append(out1_1)  # 一阶段的损失 躯干向量损失
            saved_for_loss.append(out1_2)  # 关节点热力图损失
